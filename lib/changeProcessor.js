 ...
 */
export async function processWebhookNotification(accessToken, pageToken, projectId) {
    ...
    try {
        ...
        const logs = changes
            .filter(change => {
                // フォルダのみを対象にする
                return change.file && change.file.mimeType === 'application/vnd.google-apps.folder';
            })
            .map(change => {
                const owner = change.file && change.file.owners ? change.file.owners[0].displayName : 'Unknown';
                return processChange(change, owner);
            });
